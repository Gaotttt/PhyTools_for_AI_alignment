...
class ParamGroupsManager(object):
    ...
    def is_params(params):
        ...
        Example:
            >>> models = nn.Linear(3,2)
            >>> ParamGroupsManager.is_params(models.parameters())
            (True,OrderedDict([(0,...),(1,...)]))
            >>> ParamGroupsManager.is_params(models.named_parameters())
            (True,OrderedDict([('weight',...),('bias',...)]))
            >>> ParamGroupsManager.is_params(dict(models.named_parameters()))
            (True,OrderedDict([('weight',...),('bias',...)]))
            >>> ParamGroupsManager.is_params([models.weight,])
            (True,OrderedDict([(0,...),]))
            >>> ParamGroupsManager.is_params([models.weight.data,])
            (False,OrderedDict([(0,...),(1,...)])) # split models.weight.data
        """
        try:
